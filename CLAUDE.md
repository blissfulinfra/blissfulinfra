...
## TODOs
- **Current state (2026-07-28):** the tenant/project/service hierarchy (ADR-0017) is the only model — the flat and client models were purged in the 2.0 cleanup (see CHANGELOG). The **local Kubernetes golden path shipped** ([ADR-0020](docs/adr/0020-local-kubernetes-runtime.md)): `cluster up` Terraform-provisions a per-tenant kind cluster with ArgoCD + Argo Rollouts + Gitea; `project create --runtime kubernetes` + `deploy` runs build → kind load → gitops push → ArgoCD sync → pause-based canary, drivable from the CLI and the dashboard's Canary card. The **MCP server was rebuilt as an in-process control plane** ([ADR-0021](docs/adr/0021-mcp-in-process-control-plane.md)) — it had never left the flat model — and **Cloudflare is now a promotion target** ([ADR-0022](docs/adr/0022-cloudflare-as-promotion-target.md)) via `deploy --target cloudflare` plus a new `hono` template that runs both in a container and on Workers.
- **Developer loop: live reload / watch mode.** The old `blissful-infra dev` command (removed in 2.0) provided hot reload for local development. Need to reimplement for the tenant/project/service model: `blissful-infra service watch <name>` should watch the service's source, rebuild on change, and restart the container (sub-30s feedback loop). Per-template: Spring Boot Gradle rebuild is fastest (~10s), React Vite HMR is ideal (~2-5s), Node/Hono is similar. The compose runtime is the right place to start (Kubernetes deploy loop already has ArgoCD reconciliation). Blocked on: prioritization.
- **AWS deploy target.** `deploy --target aws` is the natural follow-up to ADR-0022 and is complementary rather than redundant: ECS/Fargate suits the container-shaped `spring-boot` services that Cloudflare structurally cannot host. The dead flat-model `deploy/aws.ts` (a thin `cdk deploy --all` shell) was deleted with the dispatcher; recover it from git history if useful.
- **Cloudflare data story.** A promoted service loses Kafka and the project's shared Postgres. D1 is the declared substitute but nothing migrates schemas between the local rehearsal and production — the sharpest edge in ADR-0022.
...
**MCP server** (`packages/cli/src/server/mcp/`) exposes the tenant model and the Kubernetes golden path as tools for Claude. Run with `blissful-infra mcp`. It calls the CLI's library layer in-process rather than proxying the API server (ADR-0021), so it needs neither the dashboard nor a port. Its stdout is the JSON-RPC stream — nothing in that code path may `console.log`.
