...
The JVM is the dominant runtime in enterprise software: finance, healthcare, e-commerce and large tech. Java has been there since the 90s. Kotlin is JVM-compatible but modern: concise, null-safe and the preferred language for new JVM projects.
...
**Tradeoffs:** Dynamically typed: bugs a compiler would catch in Java surface at runtime instead. The GIL (Global Interpreter Lock) limits true CPU parallelism in CPython. Slower at runtime than compiled languages. Managing dependencies and environments (pip, venv, conda) is notorious friction.
...
**Strengths:** Very fast startup (good for serverless and CLIs), statically typed, simple language spec (small surface area), great standard library, excellent for high-concurrency network services.
...
Rust is a systems programming language with memory safety guarantees and no garbage collector. It is fast and prevents entire classes of memory bugs at compile time.
...
**Cloud equivalent:** MongoDB Atlas (first-party hosted, runs on AWS/GCP/Azure). Amazon DocumentDB is MongoDB-compatible but not the same engine: it implements a subset of the MongoDB API on a completely different storage layer. Verify behavior compatibility before treating them as interchangeable.
...
**Tradeoffs:** Data must fit in memory (with caveats). Not designed as a primary data store: data can be lost on restart unless persistence is configured. Not suited for complex queries.
...
**Use for:** Event analytics, metrics storage, user behavior data and business intelligence queries. Often deployed alongside a Postgres primary store, with data flowing in from a Kafka pipeline.
...
**Strengths:** Linear horizontal scaling, no single point of failure, tunable consistency, handles massive write volumes (IoT sensor data, click streams).
...
**Cloud equivalent:** Amazon OpenSearch Service (AWS's managed fork of Elasticsearch), Elastic Cloud (first-party hosted). Amazon OpenSearch and Elasticsearch are not identical: they diverged after AWS forked the project. API compatibility is high but not guaranteed across all versions.

#### Time-series (TimescaleDB, InfluxDB)

Databases optimized for time-stamped data: metrics, events, sensor readings and financial ticks. TimescaleDB is a Postgres extension: you use SQL and it handles time-series partitioning and compression automatically.
...
The application is split into independently deployable services, each owning a bounded domain (users, orders, payments). Services communicate over the network (HTTP/REST, gRPC, or a message broker).
...
Functions-as-a-service: you deploy code, the provider handles servers, scaling and the runtime. AWS Lambda, Google Cloud Functions and Cloudflare Workers are the common choices.
...
Code running close to the user in distributed CDN nodes around the world. Cloudflare Workers, Vercel Edge Functions, Deno Deploy and Fastly Compute are the main options.
...
**Strengths:** Decouples producers from consumers: neither needs to know about the other. Events are durable (Kafka retains the log). Multiple consumers can process the same event independently. Good for workflows that need to fan out to multiple downstream systems.
...
|---|---|---|---|
| Model | Durable log, consumer reads at its own pace | Push-based broker, messages consumed and deleted | Managed queue, messages deleted after consumption |
