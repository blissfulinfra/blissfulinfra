...
The LocalStack and AWS free-tier reasoning here is unchanged. The **blissful-infra integration is not**: the LocalStack plugin and the `lambda` command were removed in 2.0 along with the client model. LocalStack still works perfectly well on its own — you just won't get it wired up by the CLI today.
...
work I've seen up close uses both.
...
[blissful-infra has it](/getting-started). Same pattern, sized
for a laptop.
