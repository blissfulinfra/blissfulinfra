...
blissful-infra runs entirely on your laptop. Docker Desktop is the only hard dependency; the Kubernetes runtime additionally needs `kind`, `kubectl` and `terraform`, all installable with one `brew install`. There are no cloud accounts to set up and no per-hour costs ticking up while you work.
...
The Kubernetes runtime runs real canary deploys. A rollout walks 10% → 25% → 50% → 100%, pausing at each step so you can promote early or abort:
